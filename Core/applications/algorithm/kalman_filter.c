			...
			uint8_t zSize)
{
  ...
  Matrix_Init(&kf->Pminus,
	      kf->xhatSize,
	      kf->xhatSize,
	      (float*) kf->Pminus_data);
...
void Kalman_Filter_xhatMinusUpdate(KalmanFilter_t *kf)
{
  if (!kf->SkipEq1)
  {
    if (kf->uSize > 0)
    {
      ...
      kf->MatStatus = Matrix_Add(&kf->temp_vector,
				 &kf->temp_vector1,
				 &kf->xhatminus);
    }
    else
...
void Kalman_Filter_SetK(KalmanFilter_t *kf)
{
  if (!kf->SkipEq3)
  {
    ...
    kf->MatStatus = Matrix_Multiply(&kf->temp_matrix,
				    &kf->HT,
...
void Kalman_Filter_xhatUpdate(KalmanFilter_t *kf)
{
  if (!kf->SkipEq4)
  {
    ...
    kf->temp_vector1.numCols = 1;
    kf->MatStatus = Matrix_Subtract(&kf->z,
				    &kf->temp_vector,
