...
void Motor_Calc(gimbal_control_t *feedback_update)
{
	...
	//  	real -= 2 * PI ;
		...
		//motor_ready[MOTOR_YAW].output_Position=0;
	...
	//real=feedback_update->gimbal_pitch_motor.absolute_angle;
		...
		real=msp(feedback_update->gimbal_pitch_motor.absolute_angle,0,8191,-pi,pi);
    //过零点处理
	// while(tar-real > 4096)
	//  	real += 8191 ;
	// while(tar-real < -4096)
    ...
    motor_ready[MOTOR_PITCH].output=pid_calc_speed(&gimbal_yaw_speed_pid,motor_ready[MOTOR_PITCH].output_Position,motor_data[MOTOR_PITCH].speed);
		//motor_ready[MOTOR_PITCH].output=pid_calc_speed(&gimbal_yaw_speed_pid,3000,motor_data[MOTOR_PITCH].speed);
